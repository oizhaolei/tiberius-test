...
release_lnx:
	...
	mv .env.sample .env
	zip ${BIN_NAME}-v${VERSION}-x86_64-linux.zip ${BIN_NAME} README.md .env

release_win:
	cargo build --release --target=x86_64-pc-windows-msvc
	mv -v target/x86_64-pc-windows-msvc/release/${BIN_NAME}.exe ./
	mv .env.sample .env
	7z a ${BIN_NAME}-v${VERSION}-x86_64-windows.zip ${BIN_NAME}.exe README.md .env

release_mac:
	cargo build --release --target=aarch64-apple-darwin
	mv -v target/aarch64-apple-darwin/release/${BIN_NAME} ./
	mv .env.sample .env
	zip ${BIN_NAME}-v${VERSION}-x86_64-mac.zip ${BIN_NAME} README.md .env
